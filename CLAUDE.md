...
- Live baseline (`<OCI_PROFILE_CAP>` staging): 29 dashboards / 441 saved searches deploy clean. Live query validation runs at the configured `--query-lookback`; at a 14-day window 4 heavy Linux detections (`linux_dns_tunneling_detected`, `linux_ld_preload_library_hijacking`, `linux_password_file_direct_modification`, `web_server_process_spawning_shell_with_injection_characters_linux`) can exceed the 60s per-query timeout — raise `--query-timeout` or use `--skip-live-validation` (content is parse-valid; the gate is all-or-nothing). Reconcile dashboard/query counts with `queries/catalog.json` + `queries/dashboard_inventory.json` after changes.
