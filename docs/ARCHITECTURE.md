...
| `queries/*.json` | Generated top-level OCI detection queries | 447 | Produced by `scripts/convert_sigma.py` |
| `queries/apps/*.json` | App telemetry query surface | 46 | 8 Sigma-derived browser detections + 38 curated app/APM analytics |
| `queries/hunting/*.json` | Curated hunting analytics | 87 | Frequency, anomaly, scoring, incident, and correlation content |
| `queries/sentinel/*.json` | Microsoft Sentinel converted queries | 421 | Produced by `scripts/sentinel_conversion_workflow.py`; live OCI parser-passing only |
...
- There are **455 Sigma-derived OCI query artifacts** in total.
- Those 455 are split across **447 top-level detections** in `queries/` and **8 browser/app telemetry detections** in `queries/apps/`.
- The repo also ships **125 curated analytics** that are not Sigma-derived: 38 app/APM telemetry analytics and 87 hunting queries.
- The repo also ships **421 live-validated Microsoft Sentinel conversions** in `queries/sentinel/`.
...
rules/** ------------------------------------------+
           ...
           447 generated detections                    8 Sigma-derived browser queries

queries/apps/*.json (38 curated app/APM analytics) ---+
queries/hunting/*.json (87 curated analytics) --------+----> scripts/generate_catalog.py
                                                            - CATALOG.md
...
- Combined query inventory:
  - 1,001 total query artifacts/content items
  - 421 live-validated Microsoft Sentinel conversions
  - 241 MITRE ATT&CK techniques across 18 tactics
  - 24 STIG-mapped detections across 12 controls
- Dashboard layer:
  - 27 dashboards
  - 450 widget-backed saved searches
  - 91 advanced visualization widgets (`tile`, `sunburst`, `summary_table`, `line`, `bar`, `link`, and `map`)
- Demo/test data:
  - 17 NDJSON files
