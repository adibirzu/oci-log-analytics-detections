# Integrated Forge Webapp

Date: 2026-06-29

`webapp/` is the long-term Forge frontend for this repository. It replaced the old `LoganSecurityDashboardv0` sibling app as the maintained UI surface.

## Runtime Contract

- The app is a Next.js App Router project under `webapp/`.
- The only browser product surface is `/forge`; root HTML requests redirect there.
- The SIEM parser workspace remains on that surface at `/forge?view=log-samples`, with `raw`, `detections`, and `benefits` tabs selected by query parameter.
- The Resource Manager handoff remains on the same surface at `/forge?view=deployment`; its package endpoint has no tenancy or credential inputs and sends the customer to the OCI Console for the privileged apply.
- Allowed API routes are `/api/forge/session`, `/api/forge/convert`, `/api/forge/artifacts`, and `/api/health`.
- Local development reads artifacts from the repository root by default.
- Container deployments stage a minimal read-only artifact bundle with `webapp/deploy/oke/stage-detections-runtime.sh`.

## Artifacts Used by the UI

- `queries/logan_ql_reference_catalog.json`
- `queries/cross_ql_mapping_patterns.json`
- `queries/conversion_examples.json`
- `queries/ql_conversion_capability_matrix.json`
- `queries/catalog.json`
- `queries/dashboard_inventory.json`
- `queries/siem_log_examples.json`
- `test_data/manifest.json`
- `scripts/logan_workbench_convert.py`

The webapp must not hand-author query-generation logic. Arbitrary conversions go through `/api/forge/convert`, which either proxies to `LOGAN_FORGE_BACKEND_URL` or executes the bundled read-only converter script.

`queries/siem_log_examples.json` is generated by `scripts/generate_siem_log_examples.py` from reviewed service templates and existing detection-query specifications. The Log Samples workspace exposes ten OCI Logging service examples in their source envelope and ten Log Analytics detection events in both the native alarm-event shape and a stable normalized SIEM contract. Users can copy one event or download JSON/JSONL bundles without an API call.

The deployment workspace reads `queries/catalog.json` and `queries/dashboard_inventory.json` for its public content summary. In a dynamic deployment, `/api/forge/deployment-package` invokes `scripts/build_orm_stack.py` to produce a short-lived zip from committed deployment files. The endpoint accepts no request input, does not reach OCI, and does not receive credentials. The caller performs the actual plan/apply after authenticating to OCI Resource Manager.

## Security Notes

- Public Repo links point to `https://github.com/adibirzu/oci-log-analytics-detections`.
- The UI must not render tenancy names, OCIDs, public IPs, OCI profile names, backend URLs, or tokens.
- Parser examples use semantic placeholders such as `<COMPARTMENT_OCID>` and reserved documentation addresses. Never replace them with live tenancy values in committed artifacts.
- The deployment-package endpoint is deliberately credentialless. Do not add OCI credential, profile, compartment, or tenancy form fields to Forge; user approval and privileged changes belong in OCI Resource Manager.
- Production write-capable conversion belongs behind API Gateway and WAF. The frontend defaults to read-only behavior when backend secrets are absent.
- The Kubernetes manifest disables service-account token mounting, runs as a non-root user, drops Linux capabilities, uses a read-only root filesystem, and keeps backend credentials in an optional secret.

## Verification

```bash
cd webapp
pnpm typecheck
pnpm lint
pnpm test:unit:coverage
pnpm build
pnpm e2e
```

Producer-side artifact changes should also run `python scripts/generate_siem_log_examples.py --check` and `python -m pytest scripts/test_generate_siem_log_examples.py -q` from the repository root before deployment.

For OKE production rollout, use [OKE_OBSERVABILITY_RUNBOOK.md](OKE_OBSERVABILITY_RUNBOOK.md)
and [webapp/deploy/oke/README.md](../webapp/deploy/oke/README.md). The runbook
captures the platform checks from the latest Forge deployment: build for the
worker node architecture, roll out through the existing OCI Load Balancer,
verify `/forge` and `/api/health`, and confirm legacy/admin routes return
`404`.
