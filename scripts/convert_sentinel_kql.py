...
def _parser_schema_hash(dictionary_path: Path = FIELD_DICTIONARY_PATH) -> str:
    try:
...
def build_query_payload(candidate: dict, query: str, source_info: dict, live_validation_status: str) -> dict:
    ...
    return {
        ...
        "parser_schema_hash": _parser_schema_hash(),
...
from scripts.sentinel_converter_reporting import (  # noqa: E402
    _conversion_status_label,
    _emit_progress,
    _format_elapsed,
    _progress_context,
    _progress_reason,
    _progress_value,
    _safe_progress_error,
    _tier_for_result,
    build_conversion_report,
)
