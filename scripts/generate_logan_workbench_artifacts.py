    ...
    patterns = [
        {
            ...
            "source_construct": "base search predicates, index, sourcetype, raw search terms, and simple where predicates",
            "oci_mapping": "Choose the OCI Log Source from source hints, map field predicates to display fields, map raw terms to Original Log Content, and emit where for simple null/equality filters.",
            "support_level": "partial",
            "logan_commands": ["search", "where", "head"],
            "warning_behavior": "Warn when SPL macros, dashboard tokens, boolean precedence, subsearches, or eval-only where expressions need manual review.",
            "example_ids": ["spl-powershell-encoded"],
        },
        {
            "id": "spl-transform-pipeline",
            "source_language": "splunk_spl",
            "source_construct": "stats, timechart, bin/bucket _time, table, fields, rename, top, dedup, sort, head, rex, spath, transaction",
            "oci_mapping": "Map count/sum/distinct stats to stats, count timechart or _time bins to timestats, table/fields to fields, top/sort/head directly, named-capture rex and JSON spath to eval extraction steps, simple transaction grouping to link, and dedup to distinct with a lossy warning.",
            "support_level": "partial",
            "logan_commands": ["stats", "timestats", "eval", "fields", "rename", "top", "distinct", "link", "sort", "head"],
            "warning_behavior": "Warn on multivalue aggregations, dedup first-event semantics, extraction parser dependencies, transaction duration loss, and runtime commands that cannot be represented by a single Logan query.",
            "example_ids": ["spl-powershell-encoded", "spl-threat-ip-lookup"],
        },
        {
        ...
        },
        {
            ...
            "oci_mapping": "Prefer source-side parser enrichment or dashboard-level correlation. Convert simple same-source SPL transaction grouping to Logan link only when stable grouping fields exist.",
            "support_level": "unsupported",
            "logan_commands": ["stats", "link"],
            "warning_behavior": "Block true cross-table joins and sequence semantics with unsupported_join_or_sequence; mark transaction-to-link conversion lossy.",
...
def build_examples() -> dict:
    ...
    examples = [
        ...
        },
        {
            ...
            "explanation": "SPL source constraints become the Logan base search; field predicates map to OCI display fields; stats/sort/head are retained as pipe commands.",
            "warnings": ["SPL conversion covers common pipeline commands; macros, subsearches, rex/spath extraction, joins, and transactions require manual handling."],
            "support_level": "partial",
            "synthetic_log_shape": "Windows Sysmon Events with host, user, process, and command line fields.",
            "pattern_ids": ["spl-filter-to-search-where", "spl-transform-pipeline", "sort-limit-top"],
        },
        {
        ...
        },
        {
            ...
            "title": "Splunk Threat IP Lookup Timechart",
            "source_language": "splunk_spl",
            "source_query": "index=network src_ip=* | lookup threat_ips ip as src_ip OUTPUT threat | where isnotnull(threat) | timechart span=5m count as hits by src_ip",
            "expected_logan_ql": "'Log Source' = 'OCI VCN Flow Logs' and 'Source IP' is not null | lookup threat_ips 'Source IP' | where 'Threat Name' is not null | timestats span=5m count as hits by 'Source IP' | head 100",
            "explanation": "Lookups require a pre-created OCI lookup table; simple null filters are retained; count-based SPL timechart maps to Logan timestats.",
            ...
            "pattern_ids": ["spl-filter-to-search-where", "spl-transform-pipeline", "lookup-watchlist", "sort-limit-top"],
        },
        {
