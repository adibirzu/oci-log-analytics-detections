...
variable "splunk_evidence_exporter_hec_url" {
  type        = string
  description = "Reviewed HTTPS Splunk HEC event endpoint"
  default     = ""

  validation {
    condition     = var.splunk_evidence_exporter_hec_url == "" || can(regex("^https://", var.splunk_evidence_exporter_hec_url))
